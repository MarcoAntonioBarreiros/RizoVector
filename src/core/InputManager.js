export class InputManager{
 ...
 get axisX(){const k=(this.keys.has('ArrowRight')||this.keys.has('KeyD')||this.virtual.has('right')?1:0)-(this.keys.has('ArrowLeft')||this.keys.has('KeyA')||this.virtual.has('left')?1:0);return Math.max(-1,Math.min(1,k||this.gamepad.x))}
 get axisY(){const k=(this.keys.has('ArrowDown')||this.keys.has('KeyS')||this.virtual.has('down')?1:0)-(this.keys.has('ArrowUp')||this.keys.has('KeyW')||this.virtual.has('up')?1:0);return Math.max(-1,Math.min(1,k||this.gamepad.y))}
