...
export class Game{
 ...
 start(){
  this.random=new SeededRandom(this.seed);this.segmentGenerator=new SegmentGenerator(this.random,innerWidth);this.segmentManager=new SegmentManager(this.segmentGenerator);this.segmentManager.reset();this.player.reset(innerWidth/2-20,innerHeight-125);this.rootGrowth.reset();this.flow.reset();
 ...
 spawnSegment(s){
  ...
  for(const source of s.pickups)this.pickups.acquire({...source,radius:source.kind==='microbe'?30:11,phase:this.random.range(0,Math.PI*2),enteredView:source.y>=-45});for(const source of s.obstacles)this.obstacles.acquire({...source});for(const source of s.channels)this.channels.acquire({...source});for(const source of s.barriers)this.barriers.acquire(this.barrierSystem.create(source));
 }
 update(dt){
  ...
  this.input.updateGamepad();this.status.update(dt);this.player.update(dt,this.input,{width:innerWidth,height:innerHeight},this.effects,this.status);this.rootGrowth.update(dt);this.weapon.update(dt);this.microbes.update(dt);this.enemySystem.update(dt);this.flow.update(dt);this.obstacleSystem.update(dt);this.routeSystem.update(dt);this.barrierSystem.update(dt);this.effects.update(dt);
 ...
 adaptiveAssist(){if(this.player.health<27&&this.assistTimer<=0){this.assistTimer=9;this.pickups.acquire({kind:'resource',type:'carbon',x:this.player.x+this.player.width/2+(Math.random()-.5)*160,y:Math.max(80,this.player.y-220),radius:12,phase:Math.random()*6.28,enteredView:true});this.ui.center('EXSUDATO DE RECUPERAÇÃO DETECTADO',1.5)}}
 updateObjects(dt){this.projectiles.forEachActive(b=>{b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(b.life<=0||b.y<-40||b.x<-40||b.x>innerWidth+40)this.projectiles.release(b)});this.pickups.forEachActive(o=>{const waiting=!o.enteredView&&o.y<-45,scrollFactor=waiting?1:(o.kind==='microbe'?.08:.04);o.y+=this.scrollSpeed*dt*scrollFactor;if(o.y>=-45)o.enteredView=true;if(o.y>innerHeight+90||(o.enteredView&&o.flowAge>4&&o.y<-180))this.pickups.release(o)})}
