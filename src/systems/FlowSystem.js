...
export class FlowSystem{
 ...
 render(c,width,height){
  ...
  for(let lane=0;lane<8;lane++){
   ...
   c.strokeStyle=lane%4===0?'rgba(134,239,173,.18)':lane%4===1?'rgba(127,210,255,.16)':lane%4===2?'rgba(213,180,255,.15)':'rgba(255,209,115,.13)';c.lineWidth=1.5+(lane%2)*.7;c.setLineDash([9,17]);c.lineDashOffset=-t*(54+lane*4)*dir;c.beginPath();
   ...
   for(let j=0;j<7;j++){
    let u=(j/7+t*(.072+lane*.004)*dir+phase*.13)%1;if(u<0)u+=1;const y=u*height,x=baseX+Math.sin(y*.014+t*.42+phase)*amp+Math.sin(y*.006-t*.31+phase)*amp*.45,dy=dir*15,dx=(Math.cos(y*.014+t*.42+phase)*amp*.014+Math.cos(y*.006-t*.31+phase)*amp*.45*.006)*dy;
    c.strokeStyle=lane%2?'rgba(159,225,255,.52)':'rgba(155,247,190,.50)';c.lineWidth=2;c.beginPath();c.moveTo(x-dx,y-dy);c.lineTo(x,y);c.stroke();c.fillStyle=lane%2?'rgba(194,237,255,.82)':'rgba(201,255,220,.80)';c.beginPath();c.arc(x,y,2.2+(j%3)*.45,0,TAU);c.fill()
   }
  }
 ...
 renderExudates(c,height){
  ...
  for(let i=0;i<26;i++){
   const phase=i*2.399,dir=i%5===0?1:-1,u=(t*(.085+(i%4)*.009)+i/26)%1,travel=80+u*(190+(i%5)*26),x=rootX+Math.sin(u*5.4+phase+t*.35)*(18+u*44)+Math.cos(phase)*12,y=rootY+dir*travel,color=i%4===0?'#d5b4ff':i%3===0?'#7fd2ff':'#86efad',alpha=(1-u)*.62+.12;
   if(y<-30||y>height+30)continue;c.strokeStyle=this.rgba(color,alpha*.48);c.lineWidth=1.3;c.beginPath();c.moveTo(x-Math.cos(u*5.4+phase)*6,y-dir*13);c.lineTo(x,y);c.stroke();c.fillStyle=this.rgba(color,alpha);c.beginPath();c.arc(x,y,1.8+(i%3)*.5,0,TAU);c.fill()
  }
 }
 renderObjectTrails(c){
  this.game.pickups.forEachActive(o=>{const vx=o.flowVX||0,vy=o.flowVY||0,s=Math.hypot(vx,vy);if(s<12)return;const color=o.kind==='microbe'?'#bfffe1':o.type==='carbon'?'#86efad':o.type==='nitrogen'?'#d5b4ff':o.type==='phosphate'?'#ffd173':'#99abc7',scale=Math.min(34,s*.16);c.strokeStyle=this.rgba(color,.42);c.lineWidth=o.kind==='microbe'?2.2:1.5;c.beginPath();c.moveTo(o.x,o.y);c.lineTo(o.x-vx/s*scale,o.y-vy/s*scale);c.stroke()});
  this.game.enemies.forEachActive(e=>{const vx=e.vx||0,vy=e.vy||0,s=Math.hypot(vx,vy);if(s<35)return;const scale=Math.min(46,s*.055);c.strokeStyle=this.rgba(e.color,.26);c.lineWidth=Math.max(1.2,(e.radius||15)*.08);c.beginPath();c.moveTo(e.x,e.y);c.lineTo(e.x-vx/s*scale,e.y-vy/s*scale);c.stroke()})
 }
