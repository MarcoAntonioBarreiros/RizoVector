...
export class SegmentGenerator{
 ...
 generate(y,height){
  ...
  const lane=n=>(n+.5)*(this.width/7),safeLane=id===0?3:this.random.int(1,5);
  const templates={
  ...
  while(budget>0&&attempts++<25){
   ...
   let l=this.random.int(0,6);if(l===safeLane)l=(l+2)%7;
   s.enemies.push({type,x:lane(l),y:y+this.random.range(80,height-70),phase:this.random.range(0,Math.PI*2),seed:this.random.next()});budget-=cost;if(cost>=3)large++;
  }
  ...
  if(guaranteed){const x=id===0?lane(3):lane(safeLane),py=id===0?y+height*.57:y+height*.68;s.pickups.push({kind:'microbe',type:guaranteed,x,y:py,guaranteed:true})}
  else if(this.random.chance(.34))s.pickups.push({kind:'microbe',type:this.random.pick(Object.keys(MICROBES)),x:lane(this.random.int(1,5)),y:y+this.random.range(130,height-100)});
